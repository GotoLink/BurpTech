...
public class GuiAdvancedWorkbench extends GuiContainer
{
    ...
    {
        if (!workbench.hasCustomInventoryName())
            this.fontRendererObj.drawString(StatCollector.translateToLocal(BurpTechCore.configuration.blocks.blockAdvancedWorkbench.getUnlocalizedName() + ".name"), 8, 6, 4210752);
        else
            this.fontRendererObj.drawString(workbench.getInventoryName(), 8, 6, 4210752);

        this.fontRendererObj.drawString(StatCollector.translateToLocal("container.inventory"), 8, 140, 4210752);
